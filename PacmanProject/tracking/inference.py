...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # print noisyDistance , emissionModel , pacmanPosition 
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions: 
            ...
            for newPos , prob in newPosDist.iteritems():
                # Note !!!
                # here is not update 1 belief 
                # it update part of serveral beliefs !!!! 
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [] 
        nParticle2Sample = self.numParticles 
        while nParticle2Sample > len( positions ) :
            self.particles += positions 
            nParticle2Sample -= len(positions )

        self.particles += random.sample( positions , nParticle2Sample   )
    ...
    def observe(self, observation, gameState):
        ...
        if len( emissionModel ) > 0 :
            for p in self.particles :
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    allPossible[p] = belief[p] * emissionModel[trueDistance] 
                    if allPossible[p] > 0: bAllZero = False 
        else:
            bAllZero = False 
            allPossible[ self.getJailPosition() ] = 1.0
        ...
        if bAllZero:
            # corner case:  
            # When all your particles receive zero weight based on the evidence, you should resample all particles to initial distribution
            self.initializeUniformly( gameState )
            return  


        allPossible.normalize()
        # now you have reweighted 
        # you still need  resample 
        self.particles = [ util.sample( allPossible )    for i in xrange( self.numParticles  ) ]
    ...
    def getBeliefDistribution(self):
        ...
        import collections
        freq =  collections.Counter( self.particles )
        for k,v in freq.iteritems():
            belief[k] = 1.0*v / len(self.particles)
        return belief         
