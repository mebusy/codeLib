...
#include "../PluginCode/UnityConsole.h"

extern char path_prefix[];

#ifdef USE_GLYPH
extern boolean bInGame ;
extern boolean bInPlayback ;
extern cellDisplayBuffer* lastRbuf ;
extern void clearDisplayBufferOverlayInfo() ;
#endif

extern FILE * _FOPEN(const char * path, const char * mode) ;
extern int _RENAME(const char *old_filename, const char *new_filename) ;
extern int _REMOVE(const char *filename) ;

extern void setAchievement( enum featTypes type ) ;
extern void openKeyboard() ;

extern void cp(char *fromFilePath, char *toFilePath );
extern boolean isAutoSaveFile(const char* filepath) ;
//=================================
