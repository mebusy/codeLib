...
void writeHeaderInfo(char *path) {
	...
	if (!fileExists(path)) {
		recordFile = _FOPEN(path, "wb");
		if (recordFile) {
			fclose(recordFile);
		}
	}
	
	recordFile = _FOPEN(path, "r+b");
	rewind(recordFile);
	for (i=0; i<RECORDING_HEADER_LENGTH; i++) {
		putc(c[i], recordFile);
	}
...
void flushBufferToFile() {
	...
	if (locationInRecordingBuffer != 0) {
		...
		recordFile = _FOPEN(currentFilePath, "ab");
...
//	short i;
	...
	recordFile = _FOPEN(currentFilePath, "rb");
...
void loadNextAnnotation() {
	...
	annotationFile =  _FOPEN(annotationPathname, "r");
...
void initRecording() {
	...
	if (rogue.playbackMode) {
		...
		_REMOVE(currentFilePath);
		recordFile = _FOPEN(currentFilePath, "wb"); // create the file
		fclose(recordFile);
...
#if USE_AUTO_SAVE
        // autosave will always rename to default save name
        if (isAutoSaveFile( currentFilePath ) )
            break;
#endif
	...
	do {
		...
		if (getInputTextString(filePath, "Save game as (<esc> to cancel): ",
			...
			if (!fileExists(filePath) || confirm("File of that name already exists. Overwrite?", true)) {
				_REMOVE(filePath);
				flushBufferToFile();
				_RENAME(currentFilePath, filePath);
...
#if PLUGIN_TEST || NO_FILENAME_CONFIRM
        if ( false ) {
#else
...
boolean isAutoSaveFile(const char* filepath) {
    return strcmp( filepath, AUTO_SAVE_FILENAME ) ==0 ;
}
...
void parseFile() {
	...
	if (selectFile("Parse recording: ", "Recording.broguerec", "")) {
		...
		descriptionFile = _FOPEN("Recording Description.txt", "w");
