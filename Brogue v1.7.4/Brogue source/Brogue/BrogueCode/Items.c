		...
		if (theEvent.eventType == MOUSE_UP || theEvent.eventType == MOUSE_ENTERED_CELL) {
			if (theEvent.param1 >= 0
				...
				if (theEvent.eventType == MOUSE_UP  ) {
                    ...
                    // qibinyi, cancel move when choose different sidebar items
                    if( pathDestination== NULL || (cursor[0] == pathDestination[0] && cursor[1] == pathDestination[1]) )
...
boolean chooseTarget(short returnLoc[2],
	...
	do {
		...
		moveCursor(&targetConfirmed, &canceled, &tabKey, targetLoc, &event, NULL, false, true, false , NULL );
	...
	switch (theItem->kind) {
		...
		case SCROLL_ENCHANTING:
            ...
            if ((theItem->category & (WEAPON | ARMOR | STAFF | RING | CHARM))
                ...
                // qibinyi
                setAchievement( FEAT_SPECIALIST );
            }
			if (theItem->flags & ITEM_EQUIPPED) {
				equipItem(theItem, true);
			}
