...
short actionMenu(short x, boolean playingBack) {
    ...
    // qibiyi, prevent "control may reach end of non-void function" error
    return 0 ;
}
...
//					if (playerPathingMap[cursor[0]][cursor[1]] != 1
			...
			// Get the input!
			rogue.playbackMode = playingBack;
			doEvent = moveCursor(&targetConfirmed, &canceled, &tabKey, cursor, &theEvent, &state, !textDisplayed, rogue.cursorMode, true , pathDestination );
			rogue.playbackMode = false;
...
void getCellAppearance(short x, short y, uchar *returnChar, color *returnForeColor, color *returnBackColor) {
    ...
    brogueAssert(coordinatesAreInMap(x, y));
#ifdef USE_GLYPH
    displayBuffer[mapToWindowX(x)][mapToWindowY(y)].u3d_monsterID = NUMBER_MONSTER_KINDS ;
#endif
	...
	if (!playerCanSeeOrSense(x, y)
		...
		if (pmap[x][y].flags & HAS_PLAYER) {
			...
			needDistinctness = true;
#ifdef USE_GLYPH
            displayBuffer[mapToWindowX(x)][mapToWindowY(y)].u3d_monsterID = MK_YOU ;
#endif
			...
			if (player.status[STATUS_HALLUCINATING] > 0 && !(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE)) && !rogue.playbackOmniscience) {
                ...
                int randKind = rand_range(1, NUMBER_MONSTER_KINDS - 1) ;
				cellForeColor = *(monsterCatalog[ randKind ].foreColor);
#ifdef USE_GLYPH
                displayBuffer[mapToWindowX(x)][mapToWindowY(y)].u3d_monsterID = (enum monsterTypes)randKind ;
#endif
            } else {
...
#ifdef USE_GLYPH
                displayBuffer[mapToWindowX(x)][mapToWindowY(y)].u3d_monsterID = monst->info.monsterID ;
#endif
...
FuncPtrVoid _CommitDrawFunc = NULL ;

void SetCommitDrawFunction(FuncPtrVoid fp )
{
    _CommitDrawFunc = fp;
}
...
#ifdef USE_GLYPH
                if( displayBuffer[i][j].u3d_monsterID ==MK_YOU) {
                    i+=0 ;
                    //DebugLog("%d,%d,%d", displayBuffer[i][j].isOverlay ,bInGame , bInPlayback  );
                }
#endif
...
#ifdef USE_GLYPH
                         ,( displayBuffer[i][j].isOverlay || ! (bInGame || bInPlayback) ) ? -1:
                         (int)displayBuffer[i][j].u3d_monsterID
#endif
                        );
...
void clearDisplayBuffer(cellDisplayBuffer dbuf[COLS][ROWS]) {
	...
	for (i=0; i<COLS; i++) {
		for (j=0; j<ROWS; j++) {
			...
			dbuf[i][j].opacity = 0;
#ifdef USE_GLYPH
            dbuf[i][j].u3d_monsterID = NUMBER_MONSTER_KINDS ;
            dbuf[i][j].isOverlay = false ;
#endif
        }
	}
}
...
#ifdef USE_GLYPH
void clearDisplayBufferOverlayInfo()
{
    { // init dispaly overlay info
        for (int i=0; i<COLS; i++) {
            for (int j=0; j<ROWS; j++) {
                //displayBuffer[i][j].u3d_monsterID = NUMBER_MONSTER_KINDS ;
                displayBuffer[i][j].isOverlay = false ;
            }
        }
    }
}
...
#ifdef USE_GLYPH
                    displayBuffer[i][j].isOverlay = lastRbuf!=NULL && lastRbuf != &overBuf[0][0]  ;
#endif
                }
