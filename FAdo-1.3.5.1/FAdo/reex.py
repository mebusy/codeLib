...
class ParseReg1(Yappy):
    ...
    def __init__(self, no_table=0, table='.tablereg'):
        ...
        tokenize = [
                    ...
                    # ust @t to matching '\t'
                    ("@t", lambda x: ("id" , '\t')) , # qibinyi add 
                    ("@n", lambda x: ("id" , '\n')) , # qibinyi add 
                    ("@r", lambda x: ("id" , '\r')) , # qibinyi add 

                    ("@.", lambda x: ("id" , x[1] )) , # qibinyi add 0
                    (Not, lambda x: (x, x)),  # ~ compl
                    (Shuffle, lambda x: (x, x)),  # : shuffle
                    (Conj, lambda x: (x, x)),     # &  conj 
                    # (Option, lambda x: (x, x)),   # - option
                    


                    ("\?", lambda x: (Option, Option)),   # qibinyi, option - => # 
                    ("[A-Za-z0-9]", lambda x: ("id", x)),
                    ("\|", lambda x: (x, x)),  # disj
                    # ("\+", lambda x: ("|", x)), # disj  qibinyi, remove dup
                    ("\*", lambda x: (x, x)),  # star
                    ("\(|\)", lambda x: (x, x)) , #atom
                    (".", lambda x: ("id" , x[0] )) , # qibinyi add  1
                    ]

        #print 'parseReg1 tokens' 
        #for token in tokenize:
        #    print "\t" , token[0] ,  token[1]( token[0]  ) if callable(token[1] ) else ""
        # print type( Epsilon )
