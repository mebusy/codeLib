...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(call? e) 
            (let ( [v1 (eval-under-env (call-funexp e) env) ] [v2 (eval-under-env (call-actual  e) env) ] )
             (if (closure? v1)
                ( let* ( [cls_fun (closure-fun v1) ] [cls_env (closure-env v1)] 
                    [fname (cons (fun-nameopt cls_fun) v1 ) ] [fparam (cons (fun-formal cls_fun ) v2)]
                    [ext_cls_env (cons fparam (if (fun-nameopt cls_fun) (cons fname cls_env ) cls_env ) )  ]
                    ) 
                    
                    ( eval-under-env  (fun-body cls_fun) ext_cls_env )
                 )      
                (error "MUPL call applied to not-closure")
              )
             )
          ]
         [(apair? e)
